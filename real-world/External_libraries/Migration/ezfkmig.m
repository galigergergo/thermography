function migRF = ezfkmig(RF,fs,pitch,c)
%EZFKMIG f-k migration for plane wave imaging
% (Easy version)
%
% MIGRF = EZFKMIG(RFMAT,FS,PITCH) performs a f-k
% migration of the RF signals stored in the 2-D
% array RFMAT. MIGRF contains the migrated
% signals. FS and PITCH represent the sampling
% frequency (in Hz) and the pitch (in m) that
% were used to acquire the RF signals.
%
% The RF signals in RFMAT must have been
% acquired using a PLANE WAVE configuration with
% a linear array. Each column corresponds to a
% single RF signal over time acquired by a
...
% Reference
% ---------
%     Garcia D., Tarnec L. L., Muth, S., Montagnon, E., Poreé, J., Cloutier, G., 
